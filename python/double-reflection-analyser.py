...
# Make sure the data is nicely laid out in memory for transfer to GPU
data = np.ascontiguousarray(np.reshape(image_buffer,(pixels,length)))

norm = np.mean(data, axis=1)

data_normed = np.zeros_like(data)
for a in range(data.shape[0]):
    data_normed[a] = data[a] / norm[a]

med = np.median(data_normed, axis=0)
...
#hf = np.fft.rfft(hs)                                   # CPU calculation
hf = cp.asnumpy(cp.fft.rfft(cp.asarray(hs)))            # GPU calculation
